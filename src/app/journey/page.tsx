...
const STREAK_PRISTINE = 10;
const STREAK_RICH = 3;

// Luminosity thresholds
const LUMINOSITY_RADIANT = 80;
const LUMINOSITY_HIGH = 50;
const LUMINOSITY_MODERATE = 20;
...
export default function Journey() {
  ...
  const weeklyState = useMemo(() => {
    ...
    let soilHealth = 'Dry';
    if (streakLength > STREAK_PRISTINE) soilHealth = 'Pristine';
    else if (streakLength > STREAK_RICH) soilHealth = 'Rich';
    else if (streakLength > 0) soilHealth = 'Developing';

    let luminosity = 'Low';
    if (score >= LUMINOSITY_RADIANT) luminosity = 'Radiant';
    else if (score >= LUMINOSITY_HIGH) luminosity = 'High';
    else if (score >= LUMINOSITY_MODERATE) luminosity = 'Moderate';
  ...
  return (
    <div className="relative pt-32 pb-24 min-h-screen flex flex-col items-center overflow-x-hidden">
      ...
      `}} />
      <canvas ref={canvasRef} aria-hidden="true" className="fixed top-0 left-0 w-full h-full -z-10 pointer-events-none" />
      ...
      <div ref={plantRef} className="relative w-full max-w-[1200px] h-[614px] flex items-center justify-center animate-fade-rise-delayed">
        ...
        {scoreChange !== null && (
          <div
            ...
            aria-live="polite"
            aria-atomic="true"
            className={`absolute top-[5%] left-1/2 -translate-x-1/2 z-30 px-6 py-3 rounded-full font-body text-lg font-bold shadow-lg animate-bounce
              ${scoreChange > 0
        ...
        <div className="absolute top-[10%] left-[10%] z-20 animate-float-slow hidden md:block">
          <div className="glass p-8 w-64 shadow-sm rounded-xl">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-primary text-[20px] font-bold" aria-hidden="true">✧</span>
      ...
      <div className="w-full max-w-2xl px-margin-mobile mt-12 animate-fade-rise-delayed-2">
        ...
        <h2 className="font-display text-3xl text-primary text-center mb-6">Log an Action</h2>
            ...
            <div className="mt-6 w-full animate-fade-rise flex flex-col gap-4">
              {loggedAction && (
                <div className="text-center mb-1">
                  ...
                  <h3 className="font-display text-xl text-primary italic">&ldquo;{loggedAction}&rdquo;</h3>
                </div>
              )}
              <ErrorBoundary>
                {isFetchingFeedback ? (
                  ...
                  <div className="bg-gradient-to-br from-green-50 to-emerald-50/50 backdrop-blur-md rounded-2xl p-6 border border-green-200/60 shadow-sm">
                    <div className="flex items-center gap-2 mb-4">
                      <Sparkles size={20} className="text-green-600" aria-hidden="true" />
                      <h4 className="font-body text-xs font-bold text-green-800 uppercase tracking-widest">Great Choice!</h4>
                    </div>
                    <p className="font-body text-sm text-green-900 leading-relaxed mb-5">{feedback.praise}</p>
                  ...
                  <div className="bg-gradient-to-br from-amber-50 to-red-50/50 backdrop-blur-md rounded-2xl p-6 border border-amber-200/60 shadow-sm">
                    ...
                    <p className="font-body text-sm text-amber-900 leading-relaxed mb-5">{feedback.reality}</p>
                    <div className="border-t border-amber-200/50 pt-4">
                      ...
                      <div className="flex flex-col gap-2">
                        {feedback.alternatives?.map((alt: string, i: number) => (
                          <div key={i} className="flex items-start gap-2.5">
                            <XCircle size={16} className="text-amber-500 mt-0.5 flex-shrink-0" aria-hidden="true" />
        ...
        <div className="mt-8 bg-white/40 backdrop-blur-md rounded-2xl border border-primary/5 p-6 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-2">
              <span className="text-primary text-[20px] font-bold" aria-hidden="true">🗒</span>
              <h2 className="font-display text-2xl text-primary">Today&apos;s Choices</h2>
            </div>
                ...
                return (
                  <div key={act.id} className="flex items-center justify-between p-4 bg-white/70 backdrop-blur-md rounded-xl border border-primary/5 hover:bg-white transition-colors group">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-lg ${isGood ? 'bg-green-50 text-green-700 animate-pulse' : 'bg-amber-50 text-amber-700'}`} aria-hidden="true">
                        <CatIcon size={18} />
                      </div>
                      <div>
                        <h3 className="font-body text-sm font-semibold text-primary">{act.label}</h3>
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`font-body text-xs font-bold px-2.5 py-1 rounded-full ${isGood ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`} aria-label={`${isGood ? 'Plus' : 'Minus'} 10 points`}>
                        {isGood ? '+10' : '-10'} pts
                      </span>
                      <button
        ...
        <div className="md:col-span-8 relative">
          <div className="aspect-[16/9] w-full glass overflow-hidden flex items-center justify-center p-12 rounded-xl">
            <div className="w-full flex flex-col items-center text-center relative z-10">
              <span className="text-4xl text-primary/20 mb-6" aria-hidden="true">✧</span>
              <blockquote className="font-display text-3xl italic mb-4 text-primary">&ldquo;The forest is not a collection of trees, but a network of relationships.&rdquo;</blockquote>
