  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({
        insight: `Every choice adds up. Your recent log of "${activity.label}" has a footprint of ${activity.emissionsValue}kg CO2e.`,
    ...
    const equivalenciesText = EQUIVALENCIES.map(e => `${e.co2e}kg CO2e is about equivalent to ${e.equivalent}`).join('. ');

    const prompt = `
      You are an encouraging, non-shaming environmental awareness AI. 
      The user just logged an activity: Category "${activity.categoryId}", Action "${activity.label}", which is estimated at ${activity.emissionsValue}kg CO2e.
      Their current rolling weekly score is ${score}/100 (where 100 is best). Their total recent emissions are ${weeklyEmissions}kg CO2e.
      
      Here are some reference points: ${equivalenciesText}.
      
      Provide a short (1-2 sentence) encouraging insight that reframes today's specific logged activity (${activity.label}) into a relatable real-world equivalent using the reference points if helpful, but DO NOT shame the user. 
